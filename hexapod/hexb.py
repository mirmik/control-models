...
class leg_controller:
	def __init__(self, hexapod, index, reference):
		...
		self.servos[0].set_regs(
			pidspd=zencad.bullet.pid(kp=30,ki=0),
			pidpos=zencad.bullet.pid(kp=20,ki=0), 
			filt=F)
		self.servos[1].set_regs(
...
#
	...
	def go_ground(self):
		self.control_mode = "position"
		self.target_position = self.reference_position + vector3(0,0,-10)+ vector3(0,10,0)
		self.bcmode = False	
...
class hexapod:
	...
	def body_trajectory_control(self, delta):
		...
		self.body_control_target = self.body_control_target * screw(lin=(0,3,0), ang=(0,0,0)).scale(delta).to_trans()
